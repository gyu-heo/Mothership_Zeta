...
import logging
import time
from Mothership_Zeta.MZ.analysis.seqNMF.helpers import reconstruct, shift_factors, compute_loadings_percent_power, get_shapes, trim_shapes
...
def seqnmf(X, K=10, L=100, Lambda=.001, W_init=None, H_init=None,
    ...
    for i in np.arange(max_iter):
        ...
        logging.warning(f"Iteration {i}: EV cost {cost[i + 1]}")
        logging.warning(f"Iteration time {(time.time() - tic)/60} minutes")
